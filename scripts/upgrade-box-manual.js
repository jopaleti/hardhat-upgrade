...
async function main() {
    ...
    const boxProxyAdmin = await deployments.get("BoxProxyAdmin");
    const boxProxyAdminContract = await ethers.getContractAt(
        "BoxProxyAdmin",
        boxProxyAdmin.address,
        deployer,
    );
    // Upgrade!
    // Not "the hardhat-deploy way"
    // const boxProxyAdmin = await ethers.getContract("BoxProxyAdmin", deployer);
    // const transparentProxy = await ethers.getContract("Box_Proxy", boxV2.address);
